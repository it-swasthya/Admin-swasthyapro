...
const [testForm, setTestForm] =useState({
  ...
  const fetchPackages = async () => {
    try {
      ...
      const finalList = Array.isArray(decodedData.data)
        ?decodedData.data : []
  ...
  const showTestsPopup = (pkg) => {
    const testList = Array.isArray(pkg.test_data)
      ? pkg.test_data
          .map(
            (test, index) => `<li><strong>${index + 1}</strong> - ${test.test_name}</li>`
          )
          .join("")
  ...
  <Table stickyHeader aria-label="packages table" sx={{ minWidth: 2000 }}>
    <TableHead>
      <TableRow>
        {[
          "ID",
    ...
    <TableBody>
      {filteredPackages
        ...
        .map((pkg) => (
          ...
          <TableCell align="center" sx={{ py: 0.2, fontSize: "0.875rem" }}>
            ...
            <TableCell sx={{ py: 0.2, fontSize: "0.875rem" }}>
              ...
              <Button
                      ...
                      >
                        ...
                        sx={{ mr: 1 }}
                        startIcon={<AddIcon />}
                        onClick={() => {setSelectedPackage(pkg) ;setOpenTestModal(true)}}
                      >
                        Labs
                      </Button>
                      </TableCell>
  ...
  {pkg.test_data?.length > 0 ? (
    <span style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
      <Typography
        sx={{ color: "primary.main", cursor: "pointer", mr: 0.5 }}
        variant="body2"
      >
      ...
      <Tooltip title="View Tests in this packages">
        <IconButton
 onClick={() => showTestsPopup(pkg)} size="small"
          sx={{ p: 0.5 }}
        >
  ...
  {pkg.test_details ? (
    ...
    <Typography variant="body2" color="text.secondary">
      {highlightMatch("N/A")}
            ...
            <TableCell sx={{ py: 0.2, fontSize: "0.75rem" }} align="right">
  <button
    onClick={() =>
