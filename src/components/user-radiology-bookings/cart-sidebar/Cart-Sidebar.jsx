    ...
    isModalOpen && (
      <div className="fixed inset-0 z-50 flex items-start justify-end bg-black/20 backdrop-blur-sm">
        <div className="relative w-full sm:w-[400px] max-w-full bg-white h-screen shadow-xl flex flex-col rounded-l-xl">
          ...
          {/* Pay Modal */}
          <Dialog open={payModalOpen} onClose={() => setPayModalOpen(false)}>
            <DialogTitle>
            ...
            <DialogActions>
              <Button onClick={() => setPayModalOpen(false)} color="secondary">
                Cancel
              </Button>
            </DialogActions>
          </Dialog>
...
//   const dispatch = useDispatch();
//  const [totalDiscount, setTotalDiscount] = useState(0);
//   // Discount state per item -> store full object {id, name, price, discount_price}
//   const [discountPrices, setDiscountPrices] = useState(
//     cartData.reduce((acc, item) => {
//       acc[item.id] = {
//         id: item.id,
//         name: item.type_of_study,
//         price: Number(item.swasthyapro_rate) || 0,
//         discount_price: 0,
//       };
//       return acc;
//     }, {})
...
//   // Total price calculation (swasthyapro_price - discount)
//  const totalPrice = cartData.reduce((acc, item) => {
//   const discount = discountPrices[item.id] || 0;

//   // price = swasthyapro_rate
//   const price = Number(item.swasthyapro_rate) || 0;
//   const netPrice = price - discount;

//   return acc + netPrice;
// }, 0);

//   // When placing order, build array of objects {id, name, price, discount_price}
...
//             {cartData.map((item) => {
//               const entry = discountPrices[item.id] || {};
//               const discount = entry.discount_price || 0;
//               const basePrice = Number(item.swasthyapro_rate) || 0;
...
//               <div className="mt-4">
//               <label className="block mb-1 text-gray-700 text-sm">
//                 <span className="text-red-600">*</span>Total Discount
//               </label>
//               <input
//                 type="date"
//                 value={totalDiscount}
