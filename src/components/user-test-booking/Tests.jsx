...
const Tests = ({facilityName , testList , selectedTestNames , handleCheckboxChange , highlightText , searchQuery }) => {
  return (
  ...
  {testList.length === 0 ? (
    ...
    <div className="space-y-4">
      {testList.map((test) => {
        const selected = selectedTestNames.includes(test.test_name || test.type_of_study);
        return (
          <label
            key={test.id}
          ...
          >
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <span className="text-gray-800 font-medium text-base sm:text-lg">
                {highlightText(test.test_name || test.type_of_study, searchQuery)}
              </span>
            </div>
